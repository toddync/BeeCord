<script lang="ts">
    ...
    import { Check, Hash, MessageSquare, Plus, Search, Upload, Video, X } from "@lucide/svelte";
    ...
    function openChat()  { openDialog = "chat";  step = 1; resetForm(); }
    function openRoom()  { openDialog = "room";  step = 1; resetForm(); }
    function openVideo() { openDialog = "video"; step = 1; resetForm(); }
    function close()     { openDialog = null;    step = 1; resetForm(); }
    ...
    function onAvatarPicked(e: Event) {
        ...
        if (!file) return;
        if (!file.type.startsWith("image/")) { error = "Avatar must be an image file"; return; }
        if (avatarPreview) URL.revokeObjectURL(avatarPreview);
        avatarFile = file;
    ...
    async function handleCreateChat() {
        ...
        if (invites.length === 0) { error = "Select or enter at least one user"; return; }
    ...
    async function handleCreateRoom(isVideo = false) {
        ...
        try {
            ...
            const videoState = { type: "im.vector.room.set_restrict_room_membership", state_key: "", content: { type: "video_room" } };
            const avatarState = avatarMxc ? { type: "m.room.avatar", state_key: "", content: { url: avatarMxc } } : null;
            const initial_state = [avatarState, isVideo ? videoState : null].filter(Boolean);

            const res = await Matrix.client?.createRoom({
                name: roomName.trim(),
                topic: roomTopic.trim() || undefined,
                visibility: roomVisibility === "private" ? sdk.Visibility.Private : sdk.Visibility.Public,
                preset: roomVisibility === "public" ? sdk.Preset.PublicChat : sdk.Preset.PrivateChat,
                invite: allInvites(),
                ...(initial_state.length > 0 && { initial_state }),
                ...(isVideo && !avatarMxc && { room_version: "org.matrix.msc3898" }),
            }) as { room_id: string };
...
<!-- Hidden file input -->
<input bind:this={fileInput} type="file" accept="image/*" class="hidden" onchange={onAvatarPicked} />
    ...
    <DropdownMenu.Content class="bg-black/10 backdrop-blur-sm border-2">
        <DropdownMenu.Group>
            <DropdownMenu.Item onclick={openChat}>
                <MessageSquare class="stroke-[1.5] stroke-primary opacity-100" /> Start chat
            </DropdownMenu.Item>
            ...
            <DropdownMenu.Item onclick={openVideo}>
                <Video class="stroke-[1.5] stroke-primary opacity-100" /> New video room
            </DropdownMenu.Item>
        </DropdownMenu.Group>
...
{#snippet stepIndicator(current: number, total: number)}
    <div class="flex items-center gap-2 mb-1">
        {#each Array(total) as _, i}
            <div class="h-1 flex-1 rounded-full transition-colors {i + 1 <= current ? 'bg-primary' : 'bg-muted'}"></div>
        {/each}
    </div>
...
{#snippet inviteStep()}
    <div class="flex flex-col gap-3">
        ...
        {#if selectedMxids.size > 0}
            <div class="flex flex-wrap gap-1.5">
                {#each [...selectedMxids] as mxid}
                    <span class="flex items-center gap-1 rounded-full bg-primary/10 border border-primary/20 px-2.5 py-0.5 text-xs font-medium">
                        {knownUsers.find(u => u.userId === mxid)?.displayName ?? mxid}
                        <button onclick={() => removeSelected(mxid)} class="text-muted-foreground hover:text-foreground">
                            <X class="size-3" />
                        </button>
                    </span>
                {/each}
        ...
        <div class="relative">
            ...
            <Input bind:value={userSearch} placeholder="Search users…" class="pl-8" />
        </div>

        <!-- User list -->
        <div class="flex flex-col gap-0.5 max-h-44 overflow-y-auto rounded-md border border-input bg-muted/30 p-1">
            {#if filteredUsers.length === 0}
                <p class="text-xs text-muted-foreground text-center py-4">No users found</p>
            {:else}
                {#each filteredUsers as user}
                    {@const selected = selectedMxids.has(user.userId)}
                    <button
                        onclick={() => toggleUser(user.userId)}
                        class="flex items-center gap-2.5 px-2 py-1.5 rounded-sm text-sm hover:bg-muted transition-colors text-left {selected ? 'bg-primary/10' : ''}"
                    >
                        <!-- Avatar or fallback -->
                        <span class="size-7 rounded-full bg-primary/20 flex items-center justify-center text-xs font-semibold shrink-0 overflow-hidden">
                            {#if user.avatarUrl}
                                <img src={Matrix.client?.mxcUrlToHttp(user.avatarUrl, 28, 28, "crop") ?? ''} alt="" class="size-full object-cover" />
                            {:else}
                                {(user.displayName ?? user.userId)[1]?.toUpperCase()}
                            {/if}
                        </span>
                        <span class="flex-1 min-w-0">
                            <p class="font-medium truncate">{user.displayName ?? user.userId}</p>
        ...
        <div class="flex flex-col gap-1.5">
            ...
            <p class="text-xs text-muted-foreground">For users not in the list above</p>
        </div>
...
<Dialog.Root open={openDialog === "chat"} onOpenChange={(o) => !o && close()}>
    <Dialog.Content>
        <Dialog.Header>
            ...
            <Dialog.Description>
                {step === 1 ? "Who do you want to message?" : "Confirm your selection."}
            </Dialog.Description>
...
<Dialog.Root open={openDialog === "room"} onOpenChange={(o) => !o && close()}>
    <Dialog.Content>
        ...
        <div class="py-2">
            {#if step === 1}
                <div class="flex flex-col gap-4">
                    ...
                    >
                        {#if avatarPreview}
                            <img src={avatarPreview} alt="Room avatar preview" class="size-full object-cover" />
                    ...
                    {#if avatarPreview}
                        <Button variant="ghost" size="sm" class="mx-auto -mt-2" onclick={clearAvatar}>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <Input id="room-name" bind:value={roomName} placeholder="My Room" />
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <Label for="room-topic">Topic</Label>
                        <Input id="room-topic" bind:value={roomTopic} placeholder="What's this room about?" />
                    </div>
                    <div class="flex flex-col gap-1.5">
        ...
        <Dialog.Footer>
            ...
            {:else}
                <Button variant="outline" onclick={() => { step = 1; error = ""; }}>Back</Button>
                <Button onclick={() => handleCreateRoom(false)} disabled={isLoading}>
...
<Dialog.Root open={openDialog === "video"} onOpenChange={(o) => !o && close()}>
    <Dialog.Content>
        <Dialog.Header>
            ...
            <Dialog.Description>
                {step === 1 ? "Set up your video room." : "Invite people to join."}
            </Dialog.Description>
            {@render stepIndicator(step, 2)}
        </Dialog.Header>
            ...
            {#if step === 1}
                <div class="flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <Input id="video-room-name" bind:value={roomName} placeholder="My Video Room" />
                    </div>
                    {#if error}<p class="text-sm text-destructive">{error}</p>{/if}
                </div>
            {:else}
        ...
        <Dialog.Footer>
            ...
            {:else}
                <Button variant="outline" onclick={() => { step = 1; error = ""; }}>Back</Button>
                <Button onclick={() => handleCreateRoom(true)} disabled={isLoading}>
