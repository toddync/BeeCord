<script lang="ts">
    ...
    class CallWidgetDriver extends WidgetDriver {
        ...
        askOpenID(observer: SimpleObservable<IOpenIDUpdate>): void {
            ...
            Matrix.client
                ...
                .catch((err) => {
                    console.error(
                        "Failed to get OpenID token for element call widget",
                        err,
                    );
                    observer.update({ state: OpenIDRequestState.Blocked });
                });
        ...
        async cancelScheduledDelayedEvent(delayId: string): Promise<any> {
            throw new Error("not implemented");
        }
        async restartScheduledDelayedEvent(delayId: string): Promise<any> {
            throw new Error("not implemented");
        }
        async sendScheduledDelayedEvent(delayId: string): Promise<any> {
            throw new Error("not implemented");
        }
        async sendToDevice(
            eventType: string,
        ...
        async readStickyEvents(roomId: string): Promise<any> {
            return [];
        }
        ...
        async navigate(uri: string): Promise<any> {
            return;
        }
        ...
        async getMediaConfig(): Promise<any> {
            return {};
        }
        async uploadFile(file: XMLHttpRequestBodyInit): Promise<any> {
            throw new Error("not implemented");
        }
        async downloadFile(contentUri: string): Promise<any> {
            throw new Error("not implemented");
        }
    ...
    onMount(() => {
        const widget = new Widget({
            ...
            type: "m.custom",
            url: `https://call.element.io/?widgetId=${widgetId}&roomId=${encodeURIComponent(room.roomId)}&baseUrl=${encodeURIComponent(Matrix.homeserver)}&userId=${encodeURIComponent(Matrix.user_id)}&parentUrl=${encodeURIComponent("")}`,
        });
...
<iframe
    ...
    title="Element Call"
    src={"https://call.element.io/?widgetId=" +
        widgetId +
        "&roomId=" +
